...
-- КАРТА ГОТОВА. Флаг нужен тем, кто ставит объекты ПО ПОВЕРХНОСТИ: порядок запуска
-- серверных скриптов не определён, а «пощупать землю лучом» — ненадёжная замена
-- ожиданию. Базовая плита (Map.GroundTop = 2) существует с самого начала, поэтому луч
-- начинает во что-то попадать задолго до того, как здесь поднимется полотно дороги, и
-- ранний промер даёт заниженную высоту. На этом закапывались стрелки старта.
