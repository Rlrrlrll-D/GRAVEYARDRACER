...
-- зомби на бегу.
--
-- ТОЛЬКО STUDIO. Скрипт выходит на первой же строке в живой игре, и ремоуты PhotoFreeze
-- и DevZombies там не создаются вовсе — «заморозить сервер» некому и нечем. Поэтому их
-- НЕТ и в манифесте ReplicatedStorage.Net: попади они туда, Bootstrap создавал бы их
-- всем независимо от проверок здесь.
--
-- 2026-08-11: сервис ненадолго открывали живой игре (с проверкой UserId на сервере),
-- чтобы владелец мог глушить зомби при съёмке превью. ОТКАТИЛИ по требованию юзера:
-- после выхода из фото-режима в живой игре «всё ломается». Причина не найдена, и пока
-- она не найдена, дев-инструменту в живой игре не место.
--
-- Проверка прав isAllowed ниже ОСТАВЛЕНА НАМЕРЕННО, хотя при выходе выше она сейчас
-- недостижима. Если сервис когда-нибудь снова откроют, ремоуты не должны оказаться
-- беззащитными: без неё любой игрок замораживал бы сервер или сносил всех зомби — ровно
-- та дыра, что нашлась в ремоутах A-Chassis (SECURITY_AUDIT.md, раздел 2).
--
