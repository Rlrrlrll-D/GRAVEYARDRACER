...
-- ВЕРНУЛИ ТРАВУ (2026-08-01, юзер: «мне нужна трава по всему полю между могил как
-- раньше, а не два-три кустика, а это покрытие, которое вместо травы, надо убрать»).
-- «Покрытием» и был Mud, подкрашенный под жухлую траву: подмена держалась ровно
-- потому, что декорация Grass стоила 438 852 tris в кадре. Цена возвращается вместе
-- с ней — но она РОВНАЯ (рывков замер 30.07 не показал: 2230 кадров, худший 19 мс),
-- а поле из настоящих травинок отдельными деталями не собрать: чтобы получился
-- ковёр, а не кустики, их нужны десятки тысяч.
-- Откат — одна строка: вернуть Enum.Material.Mud.
MapGen.FieldMaterial = Enum.Material.Grass
