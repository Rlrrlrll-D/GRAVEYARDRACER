...
### 1.1 Размер сервера — КРИТИЧНО

**Сейчас `MaxPlayers = 60`, а мест на стартовой решётке восемь** (`PlayerFlow.MaxSlots = 8`).
На полном сервере 52 человека сидели бы в лобби и ждали заезда, в который их не возьмут:
`runCountdown` обрезает список участников по `MaxSlots`, лишние остаются смотреть.

Поставить **8** (Game Settings → Places → Server Size). Из скрипта не пишется —
`Players.MaxPlayers` read-only, это настройка места.
...
| Что | Значение | Где |
|---|---|---|
| Server Size | 8 | Places → Server Size |
