...
--   SCREAM — человеческий крик. ГЛАВНЫЙ слой, а не приправа: писк — это ЗВЕРЬ,
--            а крик — РЕАКЦИЯ, и пугает именно он. Отсюда и порядок громкостей
--            ниже: крик громче крыльев и писка втрое.
-- Пустая строка у любого из трёх = этот слой молчит.
--
-- ИСТОРИЯ ПОДБОРА, чтобы не наступить на те же грабли.
-- Сначала брали кусок «Spanish Raid Woman Screaming» и резали его на 0.55с — мол,
-- нужен только резкий вскрик. Замер настоящего роя показал, что слой честно звучит
-- (крылья 2.48с, писк 1.99с, крик 0.53с), а слышно его не было: в обрезок попадал
-- тихий разгон перед вскриком, погребённый под крыльями. Обрезку сняли, громкость
-- подняли — юзер: «крик очень слабенький, должен быть доминирующим, душераздирающим
-- и не таким коротким; писк ниже приоритетом; крик сразу, внезапно».
--
-- Та запись этого дать не могла: 1.11с всего, да ещё и с мягкой атакой — сценка из
-- фильма, а не крик ужаса. Теперь стоит профильный хоррор-крик на 4.09с, он длиннее
-- обоих остальных слоёв и договаривает, когда крылья с писком уже отшумели. Высоту
-- держим у единицы: ускорение укорачивало бы запись, а её длина здесь и нужна.
local WINGS_ID = "rbxassetid://9125386815"
local SQUEAL_ID = "rbxassetid://9117009021"
local SCREAM_ID = "rbxassetid://93232280469148" -- horror-female-scream-wet, 4.09с
...
-- ЧЁРНЫЙ СИЛУЭТ (2026-08-09, юзер: «мышей надо сделать максимально тёмными»). Цвет
-- раньше не задавался вовсе — мышь красила текстура ассета. Не чистый ноль: на ночном
-- свете абсолютно чёрное пятно теряет края и читается дырой в кадре, а не телом.
local BAT_COLOR = Color3.fromRGB(10, 9, 12)
...
local function newBat(): Bat
	...
	for _, p in parts do
		...
		-- Одного Color мало: пока на меше висит своя текстура, тон берётся с неё,
		-- поэтому сначала снимаем её. TextureID у MeshPart пишется не везде — отсюда
		-- pcall: не вышло, так останется хотя бы Color. SmoothPlastic вместо дефолта,
		-- чтобы гладкий матовый не ловил блик и мышь читалась сплошным пятном.
		if p:IsA("MeshPart") then
			pcall(function()
				p.TextureID = ""
			end)
...
local function launchOne(origin: Vector3, fast: boolean)
	...
	if fast then
		...
		-- 380, а не прежние 190 (2026-08-09, юзер: «мыши должны разлетаться быстрее
		-- вдвое»). Это скорость ПРОНОСА — того куска, где стая, дойдя до лица, уходит
		-- за спину. Хвост по времени (0.14с в bat.life) не трогаем: при вдвое большей
		-- скорости мыши за те же доли секунды покрывают вдвое большее расстояние, то
		-- есть именно разлетаются быстрее, а не просто дольше висят в кадре.
		bat.speed = 380
...
batScare.OnClientEvent:Connect(function(origin: Vector3, count: number, kind: string)
	...
	if kind == "swarm" then
		...
		if SCREAM_ID ~= "" then
			-- Высота гуляет чуть-чуть и вокруг единицы: разброс нужен, чтобы третий
			-- скример за заезд не звучал заученно, но растягивать или комкать запись
			-- нельзя — её длина и есть та самая «душераздирающесть».
			--
			-- Никаких задержек и нарастаний: крик стартует В ТОТ ЖЕ КАДР, что и рой.
			scream.PlaybackSpeed = 0.97 + math.random() * 0.1
			scream:Play()
		end
		for _ = 1, count do
