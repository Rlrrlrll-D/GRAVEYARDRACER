...
--            опасность сам. «Woman Screaming» из той же библиотеки ProSoundEffects.
-- Пустая строка у любого из трёх = этот слой молчит.
--
-- ЗАПИСЬ КРИКА ИГРАЕМ ЦЕЛИКОМ, И ЭТО НЕ МЕЛОЧЬ. Сначала она обрезалась на 0.55с —
-- «взять только резкий вскрик». Замер настоящего роя показал, что слой честно
-- звучит (крылья 2.48с, писк 1.99с, крик 0.53с), а юзер его всё равно не слышал: в
-- обрезанный кусок попадал тихий разгон перед вскриком, и под крыльями с писком от
-- крика не оставалось ничего. Без обрезки запись и так короткая — 1.11с, а на
-- поднятой высоте и того меньше.
...
-- Громче остальных слоёв намеренно: крылья и писк — фон испуга, крик — его вершина.
scream.Volume = 1.25
...
batScare.OnClientEvent:Connect(function(origin: Vector3, count: number, kind: string)
	...
	if kind == "swarm" then
		...
		if SCREAM_ID ~= "" then
			-- Выше и резче оригинала, разброс — чтобы третий скример за заезд не звучал
			-- заученно. Высота заодно укорачивает запись: на 1.25 от 1.11с остаётся 0.89с.
			scream.PlaybackSpeed = 1.18 + math.random() * 0.16
			scream:Play()
		end
		for _ = 1, count do
