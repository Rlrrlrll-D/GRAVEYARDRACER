...
-- РУЧКА РАЗМЕРА HUD. Крутить надо ЕЁ, а не потолок: потолок сам по себе ничего не
-- увеличивает. Множитель считается как «вьюпорт ÷ опора», потолок его только
-- срезает сверху — в окне 1530×561 множитель 0.71, и поднимать потолок хоть до
-- тройки бессмысленно, до него всё равно не достаёт (проверено юзером: поставил 3
-- и не увидел разницы).
--
-- HUD_GAIN множит ГОТОВЫЙ множитель, поэтому работает в любом окне:
--   1.0 — прежний размер (плашка 250×46 на полном ПК-экране, 13% ширины);
--   1.5 — нынешний: 375×69, около 19% ширины. На телефоне доля растёт так же,
--         то есть пропорция «как на ПК» сохраняется — ровно этого и добивались.
local HUD_GAIN = 1.5
...
-- fitToScreen умеет только «вписать и зажать», отдельной ручки усиления у него нет.
...
-- 0.03 подобрано юзером живьём (2026-08-10, ползунок «Толщ» в NeonTune) вместе с
-- блюмом: Intensity 0.50, Threshold 0.35 — см. EnvironmentConfig/AtmosphereSetup. Линия
-- стала вдвое тоньше прежней, а ореол вернулся за счёт блюма, а не за счёт цвета.
--
-- ЭТА ТОЛЩИНА РАБОТАЕТ ТОЛЬКО ПОКА ЖИВ БЛЮМ, и это осознанное ограничение игры
-- (2026-08-10, решение юзера). Roblox гасит постобработку на низких уровнях качества
-- графики, а Neon сам по себе ореола не даёт — он лишь плоская яркая заливка. Проверено
-- в опубликованной игре: на авто-качестве свечения нет ни у черепов, ни у стрелок
-- старта, на максимальном возвращается. Волосок в 0.03 studs без ореола не тускнеет, а
-- почти пропадает, поэтому игра рассчитана на среднее+ качество.
--
-- ЧТО НЕ СРАБОТАЕТ, если соберёшься чинить (проверено пробой 2026-08-10): гало из
-- граней ОДНОГО меша не построить. Альфа в EditableMesh:AddColor(Color3, alpha) на
-- материале Neon не применяется вовсе — грани с альфой 1.0 и 0.25 отрисовались
-- одинаково. Цвет граней применяется, но тусклая грань выходит НЕПРОЗРАЧНОЙ ТЁМНОЙ, то
-- есть вокруг черепа лёг бы грязный тёмный контур поверх фар и фонарей. Ореол
-- геометрией возможен только отдельными деталями с настоящим Transparency — по мешу на
-- слой (бюджет EditableMesh это выдержит, см. buildRibbon).
