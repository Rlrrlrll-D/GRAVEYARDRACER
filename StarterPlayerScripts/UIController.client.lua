...
-- Бело-голубой, как просил юзер, но НАСЫЩЕННЕЕ прежнего (196,228,255). Почти белый
-- неон читается блёкло: у стрелок старта цвет (110,255,170) — с провалом по одному
-- каналу, и именно провал даёт «неоновость». Здесь тот же приём в синем.
local SKULL_COLOR = Color3.fromRGB(150, 212, 255)
...
-- Было 0.34, то есть плашка в покое стояла на 0.44 — втрое прозрачнее стрелки, и
-- рядом с ней эффект читался как «не тот». Теперь 0.12 → 0.25: чужие чекпоинты всё
-- ещё тише своего, но это уже тот же неон, а не намёк на него.
local SKULL_FADE_IDLE = 0.12
