...
-- // Кому доступно -----------------------------------------------------------
-- ПУСТО = ТОЛЬКО STUDIO, И ТАК ЭТО И ДОЛЖНО ОСТАТЬСЯ.
--
-- 2026-08-11: список открывали владельцу (UserId 8110001559), чтобы снимать превью в
-- живом клиенте — там F11 даёт полный экран без ленты редактора. ОТКАТИЛИ по требованию
-- юзера: после выхода из режима в живой игре «всё ломается». Причина не разобрана, и
-- пока она не найдена, режиму в живой игре не место — риск поймать это на игроках выше
-- любого удобства съёмки.
--
-- Плюс отдельный довод против: плашку Roblox сверху слева в живом клиенте всё равно не
-- спрятать (см. hideCoreGui), то есть чистого кадра там и не получалось. В Studio
-- получается, и разрешение задаётся через tools/photo-window.ps1.
--
-- Захочешь вернуть — сперва найди, что именно не восстанавливается на выходе (leave()
-- возвращает камеру, интерфейс, блюр, имена, CoreGui и управление), и почини это.
