...
-- PLAY закрывает заставку и возвращает игрока в мир, OPTIONS открывает
-- опции-надгробие (существующий OptionsMenu).
--
...
wind:Play()
TweenService:Create(blur, TweenInfo.new(1.2), { Size = 7 }):Play()
...
optBtn.Activated:Connect(function()
	...
	if om and om:IsA("ScreenGui") then
		om.Enabled = true -- заставка прячет HUD; для опций временно возвращаем
		local stone = om:FindFirstChild("Tombstone")
		if stone and stone:IsA("GuiObject") then
			stone.Visible = true
		end
	end
end)
