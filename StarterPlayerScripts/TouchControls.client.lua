...
-- уходить слишком низко: на маленьком экране кнопки лучше займут больше доли, чем
-- станут нежимаемыми.
--
-- НИЖНИЙ ЗАЖИМ ОПУЩЕН 0.8 → 0.45 (2026-09-04, по снимку с телефона юзера). 0.8
-- ставился в расчёте на вьюпорт 500–700 точек, а телефон отдаёт ≈265: стек из трёх
-- кнопок (296 опорных точек) при 0.8 занимал 237 из 265, то есть ПОЧТИ ВЕСЬ ЭКРАН
-- по высоте, и гашетка налезала на плашку LIVES. При 0.45 тот же стек — половина
-- высоты, а кнопка газа выходит около 67×43 точек, это порядка 17×11 мм на экране
-- телефона: палец попадает с запасом (нижняя граница удобного нажатия ≈9 мм).
local REF = 700
...
local function applyScale()
	...
	local s = math.clamp(cam.ViewportSize.Y / REF, MIN_SCALE, MAX_SCALE)
	fit.Scale = s
