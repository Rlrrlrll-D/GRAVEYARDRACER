...
local function applyMobileProfile()
	...
	-- ШАГ 5: ПОСТ-ЭФФЕКТЫ. В ЗАЕЗДЕ гасим цветокоррекцию — полноэкранный проход,
	-- заметная доля кадра. В ЛОББИ возвращаем: сцена там статична, кадров хватает.
	-- BlurEffect не трогаем — им заведует LobbyUI, он гаснет на старте сам.
	-- BLOOM В ЗАЕЗДЕ ОСТАВЛЕН (2026-09-13). Раньше гасили и его, и вместе с ним на
	-- телефоне пропадал ореол вокруг ВСЕГО неона: черепов-чекпоинтов, трассеров,
	-- вспышек стволов (юзер: «свечения на дроби нет, на многоствольнике не очень»).
	-- После перевода эффектов выстрела на пул фризы ушли, кадр освободился — пробуем
	-- вернуть блюм. Если телефон снова просядет — первым кандидатом обратно в этот
	-- список идёт BloomEffect. На самом низком авто-качестве Roblox блюм вылетает и
	-- без нас — см. разбор в AtmosphereSetup.
