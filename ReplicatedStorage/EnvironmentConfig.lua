...
local EnvironmentConfig: EnvironmentConfigType = {
	...
	Evening = {
		ClockTime = 17.3,
		-- Держим ТУ ЖЕ ДОЛЮ от ночной плотности (вечер ≈ половина ночи) — иначе туман
		-- появлялся бы рывком к ночи. Ночь разредили с 0.85 до 0.45, поэтому и здесь
		-- пересчёт в той же пропорции: 0.44 × 0.53 ≈ 0.23.
		Density = 0.23,
		Brightness = 2.15,
	...
	Dusk = {
		...
		Density = 0.28,              -- та же пропорция, что у вечера: дымка реже ночной
		Brightness = 2.1,
