...
local EnvironmentConfig: EnvironmentConfigType = {
	...
	Atmosphere = {
		...
		-- ТУМАН РАЗРЕЖЕН (2026-08-08, юзер: «слишком густо»). Было 0.85/6.5 — на ночном
		-- свете это давало ровное молоко: не читались ни полотно дороги под колёсами, ни
		-- ряды могил, ни надпись на воротах, только силуэты ближних деревьев. Выбрано
		-- живьём из четырёх ночных кадров с одной точки (0.85/6.5, 0.60/4.0, 0.45/3.0,
		-- 0.30/2.0). Ниже не пошли осознанно: ночная Brightness всего 0.45, и дымка
		-- заодно работает подсветкой — на 0.30/2.0 дорога проваливалась в чёрное, и
		-- «густо» сменилось бы на «не видно».
		-- ЗАБЕЛЕНО «СЛЕГКА» (2026-09-04, выбор юзера из трёх ночных кадров с одной
		-- точки: 150,160,170/0.45 — 178,185,192/0.50 — 205,209,214/0.55; взят средний).
		-- Тронуты ТРИ величины вместе, и порознь они не работают:
		--   Density  0.45 → 0.50 — сколько мглы вообще;
		--   Color    светлее — сам цвет дымки на свету;
		--   Decay    40,45,60 → 62,68,82 — цвет той её части, что в ТЕНИ. Пока Decay
		--            оставался почти чёрно-синим, поднятый Color давал не «бело», а
		--            «синее»: ночью большая часть дали освещена как раз слабо.
		-- Выше не пошли: на 205/0.55 дальние надгробия и кресты пропадают целиком, а
		-- по ним игрок читает, куда идёт трасса.
		Density = 0.50,
		-- Offset НЕ откатывали вместе с плотностью: он про высоту, а не про густоту —
		-- туман по-прежнему стоит на уровне глаз, а не стелется по земле.
		Offset = 0.40,               -- было 0.25: туман поднялся к уровню глаз
		Color = Color3.fromRGB(178, 185, 192),
		Decay = Color3.fromRGB(62, 68, 82),
		Glare = 0,
		Haze = 3.0,                  -- было 6.5, см. Density выше
		...
		-- 0.55 подобрано юзером живьём (2026-08-10) вместе с порогом 1.35 в
		-- AtmosphereSetup и толщиной линии черепа 0.08. Прежняя единица подбиралась под
		-- ТОЛСТУЮ линию и полную яркость; на тонкой линии с пониженным порогом столько
		-- уже не нужно. Влияет на ВСЮ сцену — фары, фонари, стрелки старта.
		BloomIntensity = 0.50,
		StarCount = 4000,
	...
	Evening = {
		ClockTime = 17.3,
		-- ПРОПОРЦИЮ ОТ НОЧИ ПРИШЛОСЬ БРОСИТЬ (2026-08-09, юзер: «тумана нет»). Сначала
		-- вечер пересчитали в той же доле, что была (вечер ≈ половина ночи): 0.44 × 0.53
		-- ≈ 0.23. Но половина от УЖЕ разреженной ночи — это почти чистый воздух, а лобби
		-- и старт заезда игрок видит именно в вечернем свете, и туман оттуда пропал
		-- совсем. Теперь вечер почти вровень с ночью: дымка есть с первой секунды.
		-- Цена — туман больше не густеет к ночи, он примерно постоянен. Если захочется
		-- вернуть нарастание, поднимать надо НОЧЬ, а не ронять вечер.
		-- +0.05 вместе с ночью (2026-09-04, «слегка забелить»): дуга интерполирует
		-- ТОЛЬКО Density, поэтому поднимать её надо на всех опорах разом — иначе
		-- вечер остался бы прозрачнее ночи и на старте заезда дымка снова «пропала».
		Density = 0.45,
		Brightness = 2.15,
	...
	Dusk = {
		...
		Density = 0.50,              -- вровень с вечером и ночью, см. Evening.Density
		Brightness = 2.1,
