...
local GameConfig: GameConfigType = {
	...
	Zombie = {
		...
		-- дробь 8×14 в упор, гатлинг 9×15/с. HP ×1.5 от первой раскладки (юзер 2026-09-13:
		-- «быстро помирают»): шамблер — 3 пули / 5 дробин / 7 очередью, гуль — 5 / залп в
		-- упор / 11, брут — 12 пуль / 2–3 залпа / 27 очередью. Очки ранга — 1 за любого.
		Tiers = {
			{ id = "shambler", weight = 55, scaleMin = 1.275, scaleMax = 1.5, hp = 60, walkSpeed = 10, attackDamage = 5, bones = 4 },
			{ id = "ghoul", weight = 30, scaleMin = 1.5, scaleMax = 1.8, hp = 100, walkSpeed = 10, attackDamage = 5, bones = 6 },
			{ id = "runner", weight = 7, scaleMin = 1.1, scaleMax = 1.2, hp = 40, walkSpeed = 15, attackDamage = 4, bones = 5, maxGap = 12 },
			-- maxGap: не реже, чем раз в столько спавнов — чистый жребий 8% мог не дать ни
			-- одного брута за заезд (юзер 2026-09-13)
			{ id = "brute", weight = 8, scaleMin = 2.025, scaleMax = 2.175, hp = 240, walkSpeed = 8, attackDamage = 8, bones = 15, ramImmune = true, maxGap = 11 },
		},
	...
	Economy = {
		-- ПЕРЕВЕС В СТОРОНУ ОТСТРЕЛА (PLAN_SHOP §3, юзер 2026-09-13): платим за зомби, а не
		-- за езду. Чекпоинт 2 → 1: 12 × 3 круга = 36 за заезд. Зомби — по типу
		-- (Zombie.Tiers.bones: шамблер 4, гуль 6, раннер 5, брут 15; в среднем ≈ 5.5),
		-- пятнадцать сбитых ≈ 80. Финиш 25, победа ещё 75. Победный заезд со стрельбой
		-- ≈ 215, доехал вторым ≈ 140, победа без единого выстрела ≈ 136.
		BonesPerZombie = 5, -- запасная цена, если у зомби нет типа
		BonesPerFinish = 25,
		BonesPerWin = 75,
		BonesPerCheckpoint = 1,
		-- ПРЕДОХРАНИТЕЛЬ: за заезд оплачиваются не больше стольких зомби — иначе выгоднее
		-- встать у могил и расстреливать спавн, не двигаясь по трассе. Сверх потолка
		-- счётчики (HUD, итоги, значок) растут, кости — нет. Счётчик RaceZombies
		-- обнуляет MatchManager на старте заезда.
		ZombieBonesCap = 40,
	},
	Ranks = {
		...
		-- (2026-09-11: «я не увидел черепов сзади и сбоку — пофикси»). С 2026-09-13 на
		-- бортах вместо черепа — ИМЯ РАНГА буквами с ленты (RankSkull.Bodies: zone.text),
		-- череп остаётся на капоте и корме: «на капоте имена читаются плохо».
