...
## 4. Плей-тест — ПРОЙДЕН 2026-08-11

Правки лежат в шаблоне машины, а он собирается только в игре, поэтому проверялись
руками. Юзер подтвердил: место сохранено, машина едет, звук и дым на месте.

Значит зажимы выставлены верно и честной работе не мешают: `newSound` и `updateSound`
проходят проверку `SoundId` и клампы громкости, `UpdateSmoke` укладывается в `RATE_MAX`.
Проверка водителя не съедает первый `newSound` в момент посадки — тот самый случай,
ради которого пустая машина намеренно оставлена без проверки владельца.

Если правки понадобится повторить (например, после обновления A-Chassis или замены
шаблона машины) — сток лежит в разделе 3, новые версии восстанавливаются из истории git
по коммиту `4079520`, где они описаны целиком.
